...
function buildPdf(quote, logoPath) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor("#879f98").fontSize(8).font("Helvetica").text("DEVIS", titleX, 22);
    doc.fillColor("#ffffff").fontSize(18).font("Helvetica").text(quote.quote_number || "", titleX, 33);
    ...
    doc.fillColor("#879f98").fontSize(8).font("Helvetica")
      .text(`Date : ${fmtDate(quote.quote_date)}`, 0, 24, { align: "right", width: W - M })
      .text(`Valable jusqu'au : ${validityDate}`, 0, 36, { align: "right", width: W - M })
      .text(quote.is_free ? "Devis gratuit" : "Devis payant", 0, 50, { align: "right", width: W - M });
    ...
    /* ── TRAVAUX ── */
    if (quote.work_start_date || quote.work_duration) {
      doc.fillColor(GRAY).fontSize(7.5).font("Helvetica-Bold")
        .text("DÉTAILS DE LA PRESTATION", M, y, { characterSpacing: 0.8 });
      y += 14;
      const detail = [
        quote.work_start_date ? `Début prévu : ${fmtDate(quote.work_start_date)}` : null,
        quote.work_duration   ? `Durée estimée : ${quote.work_duration}` : null,
      ].filter(Boolean).join("   —   ");
    ...
    if (items.length > 0 || quote.labor_description) {
      doc.fillColor(GRAY).fontSize(7.5).font("Helvetica-Bold")
      ...
      if (quote.labor_description) {
        ...
        doc.font("Helvetica").text(quote.labor_description, { width: contentW - 8 });
        if (quote.labor_price_type) {
          y += 14;
          const typeLabel = quote.labor_price_type === "hourly" ? "Taux horaire" : "Forfait";
          doc.fillColor(GRAY).fontSize(8).text(`${typeLabel} : ${fmtAmt(quote.labor_price)}`, M + 4, y);
        }
        y += 16;
      }

      if (parseFloat(quote.travel_expenses) > 0) {
        doc.fillColor(GRAY).fontSize(8).font("Helvetica")
          .text(`Frais de déplacement : ${fmtAmt(quote.travel_expenses)}`, M, y);
        y += 14;
      }
    ...
    drawTotal("Total HT", fmtAmt(quote.total_ht));
    drawTotal(`TVA ${quote.tva_rate} %`, fmtAmt(quote.total_tva));
    y += 4;
    drawHLine(doc, y, GREEN, totX, totX + totW);
    y += 8;
    drawTotal("Total TTC", fmtAmt(quote.total_ttc), true);
    y += 20;

    /* ── CONDITIONS ── */
    const conditions = [
      quote.payment_conditions  ? `Paiement : ${quote.payment_conditions}` : null,
      quote.delivery_conditions ? `Livraison : ${quote.delivery_conditions}` : null,
      quote.sav_conditions      ? `SAV : ${quote.sav_conditions}` : null,
    ].filter(Boolean);
    ...
    if (quote.notes) {
      ...
      doc.fillColor("#555").fontSize(8.5).font("Helvetica")
        .text(quote.notes, M, y, { width: contentW });
    ...
    doc.fillColor(GREEN).fontSize(11).font("Helvetica-Bold")
      .text("Bon pour accord", M, y + 12, { align: "center", width: contentW });
    doc.fillColor(GRAY).fontSize(8).font("Helvetica")
      .text("Lu et approuvé — Signature précédée de la mention « Bon pour accord »", M, y + 28, { align: "center", width: contentW });
    const sigY = y + 52;
    const sig1X = M + 60;
    ...
    doc.fillColor(GRAY).fontSize(7.5).font("Helvetica")
      .text(`Devis émis via Proxilio — proxilio.fr · Valable jusqu'au ${validityDate}`, M, footerY, {
